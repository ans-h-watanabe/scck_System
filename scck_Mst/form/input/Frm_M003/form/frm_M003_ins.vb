...
Public Class frm_M003_ins
    ...
    Public Sub New(P1 As String, P2 As Form)
        InitializeComponent()
        '
        Me.Text = P1
        '
...
EXIT_FUNCTION:
    ...
    Private Function FNC_SCR_CHK() As Boolean
        ...
        Try
            ...
            End If
            '
            If TXT_部コード.Text.Equals("") OrElse TXT_部コード.Text.Equals("0") Then
                D_ERR.FNC_ERR_ADD(TXT_部コード, C_MSG_MST_INP, GcStylePlus1)
                '
                STT_Msg.Text = "" : D_RTN = False
            End If
            '
            'KEY項目チェック
            If Not D_MOD.Equals("A") AndAlso Not (D_KEY(0).Equals(TXT_部コード.Text)) Then
...
EXIT_FUNCTION:
    ...
    Private Function FNC_CMD_EXC() As Boolean
        ...
        Try
            ...
            End If
            '
            D_SQL = "SPC_M003_ACT1 " & "'" & D_MOD & "'," &
                    FNC_UPD_USR() & "," &
                    C_COM_COD & "," &
                    FNC_SQL_VAL(TXT_部コード) & "," &
                    FNC_SQL_VAL(TXT_名称) & "," &
                    FNC_SQL_VAL(TXT_略称)
            '
            ...
            If P_EXC_TBL.Rows.Count <> 0 Then
                ...
                For I = 0 To P_EXC_TBL.Rows.Count - 1 Step 1
                    Select Case P_EXC_TBL.Rows(I).Item("ERR_ID")
                        Case "9999" : D_ERR.FNC_ERR_ADD(TXT_部コード, P_EXC_TBL.Rows(I).Item("内容"), GcStylePlus1)
                        Case "1" : D_ERR.FNC_ERR_ADD(TXT_部コード, P_EXC_TBL.Rows(I).Item("内容"), GcStylePlus1)
                        Case "2" : D_ERR.FNC_ERR_ADD(TXT_部コード, P_EXC_TBL.Rows(I).Item("内容"), GcStylePlus1)
                        Case "1001" : D_ERR.FNC_ERR_ADD(TXT_部コード, P_EXC_TBL.Rows(I).Item("内容"), GcStylePlus1)
                    End Select
                Next
                '
                STT_Msg.Visible = True : D_ERR.FNC_SHW_ERR(Me, C_MSG_ERR_TTL) : GoTo EXIT_FUNCTION
            Else
                ...
                '
                STT_Msg.Text = C_MSG_CMD_CPT : TXT_部コード.Focus() : D_RTN = True
            End If
    ...
    Private Sub FNC_DSP_RPT()
        ...
        Try
            If FNC_GET_UES_LVL(Me, D_RTN) Then
                Dim NewMDIChild As New frm_M003_rpt(Me, D_RTN, D_Main)
                '
...
EXIT_FUNCTION:
    ...
    Private Sub FNC_DSP_FND()
        '
        Try
            D_SQL = "SPC_M003_FND1 1,"
            '
            Dim NewMDIChild As New frm_M003_fnd(0, D_SQL, True, "", "", P_USR, Me)
            '
            'Me.Hide()
            '
            NewMDIChild.MdiParent = D_Main
...
EXIT_FUNCTION:
    ...
    Public Sub FNC_RCV_LST(ByVal P1 As Integer, ByVal P2 As DataTable, ByVal P3 As Integer)
        ...
        Try
            ...
            Select Case P1
                Case 0
                    TXT_部コード.Text = FNC_CNV_NUL(P2.Rows(P3).Item("部コード"), 0)
    ...
    Private Sub FNC_SET_DAT(ByRef P1 As DataTable, ByVal P2 As Integer)
        ...
        Try
            ...
            If P1.Rows.Count <> 0 Then
                TXT_部コード.Text = FNC_CNV_NUL(P1.Rows(P2).Item("部コード"), 0)
...
EXIT_FUNCTION:
    ...
    Private Sub FNC_KEY_REF()
        '
        Try
            If Not (D_KEY(0).Equals(TXT_部コード.Text)) Then
                D_KEY(0) = TXT_部コード.Text

                D_SQL = "SPC_M003_INQ1 1," & C_COM_COD & "," & FNC_SQL_VAL(TXT_部コード)
                '
    ...
    Private Sub FNC_MOV_REF(ByVal P1 As String)
        ...
        Try
            If IsNothing(P_FND_TBL) OrElse P_FND_TBL.Rows.Count = 0 Then
                D_SQL = "SPC_M003_MOV1 " & "'" & P1 & "'," & C_COM_COD & "," & FNC_SQL_VAL(TXT_部コード)
                '
                D_TBL = FNC_GET_TBL(D_SQL)
                '
                FNC_SET_DAT(D_TBL, 0)
                ...
                TXT_XID.Text = 0
            Else
                TXT_部コード.Text = FNC_CNV_NUL(CLng(FNC_FND_XID(P1, P_FND_IDX, 1, "部コード")), 0)
                '
                'XID表示
...
EXIT_FUNCTION:
    ...
    Private Sub FNC_PRG_END()
        ...
        Try
            ...
            '
            CType(D_Main, Object).FNC_WIN_DEL(Nothing, Me)
            '
            'XID関連
            '
...
EXIT_SUB:
    ...
    Private Sub TXT_部コード_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TXT_部コード.Leave
        '
        Try
            If Not (D_KEY(0).Equals(TXT_部コード.Text)) Then
                '
